...
async def en_ru_message(message: types.Message, bot: Bot, state: FSMContext):
    ...
    tasks.en_ru_messageToVoice.apply_async(args=[message.text, message.chat.id, API_TOKEN, msg_id_for_reply, sticker_message_id, user_chat_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"✍️✍️ {user.id}-{mention} xabar keldi! [EN-RU] \n\n"
               f"{html.code(message.text)}\n")
    await bot.send_message(chat_id=6956376313, text=content, parse_mode="HTML")


async def en_ru_voice(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    msg_id_for_reply = message.message_id
    tasks.en_ru_voiceToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, sticker_message_id, voice_file_id, user_chat_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"🗣🗣 {user.id}-{mention} ovozli xabar keldi! [EN-RU]")
    await bot.send_voice(chat_id=6956376313, voice=voice_file_id, caption=content, parse_mode="HTML")


async def en_ru_video(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.en_ru_videoToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, sticker_message_id, video_file_id, caption_of_video])

    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"📹📹 {user.id}-{mention} video xabar keldi! [EN-RU]")
    await bot.send_video(chat_id=6956376313, video=video_file_id, caption=content, parse_mode="HTML")



async def en_ru_video_note(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.en_ru_videoNoteToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, video_note_file_id, sticker_message_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"🔘🔘 {user.id}-{mention} dumaloq xabar keldi! [EN-RU]")
    await bot.send_message(chat_id=6956376313, text=content, parse_mode="HTML")
    await bot.send_video(chat_id=6956376313, video=video_note_file_id, parse_mode="HTML")


async def en_ru_audio(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.en_ru_audioToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, sticker_message_id, audio_file_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"🎙️🎙️ {user.id}-{mention} audio xabar keldi! [EN-RU]")
    await bot.send_audio(chat_id=6956376313, audio=audio_file_id, caption=content, parse_mode="HTML")


async def en_ru_photo(message: types.Message):
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
        response = await bot.send_message(message.chat.id, random_sticker)
        sticker_message_id = response.message_id
    ...
    user = await get_user(message.chat)
    tasks.en_ru_photoToVoice.delay(api_token=API_TOKEN,
    ...
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
    content = (f"🖼🖼 {user.id}-{mention} rasmli xabar keldi! [EN-RU]")
