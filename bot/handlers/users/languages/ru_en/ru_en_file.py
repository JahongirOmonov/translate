...
async def ru_en_message(message: types.Message, bot: Bot, state: FSMContext):
    ...
    tasks.ru_en_messageToVoice.apply_async(args=[message.text, message.chat.id, API_TOKEN, msg_id_for_reply, sticker_message_id, user_chat_id])
...
async def ru_en_voice(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    msg_id_for_reply = message.message_id
    tasks.ru_en_voiceToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, sticker_message_id, voice_file_id, user_chat_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
...
async def ru_en_video(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.ru_en_videoToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, sticker_message_id, video_file_id, caption_of_video])
...
async def ru_en_video_note(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.ru_en_videoNoteToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, video_note_file_id, sticker_message_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
...
async def ru_en_audio(message: types.Message, bot: Bot, state: FSMContext):
    user = await get_user(message.chat)
    user_chat_id = user.chat_id
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
    ...
    tasks.ru_en_audioToVoice.apply_async(args=[file_path, message.chat.id, API_TOKEN, msg_id_for_reply, user_chat_id, sticker_message_id, audio_file_id])
    mention = f"<a href='tg://user?id={user.chat_id}'>userdan</a>"
...
async def ru_en_photo(message: types.Message):
    random_sticker = random.choice(sticker_file_ids)
    if random_sticker == "⏳":
        response = await bot.send_message(message.chat.id, random_sticker)
        sticker_message_id = response.message_id
    ...
    user = await get_user(message.chat)
    tasks.ru_en_photoToVoice.delay(api_token=API_TOKEN,
